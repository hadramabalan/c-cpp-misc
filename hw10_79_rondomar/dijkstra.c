...
{
	dijkstra_t* d = (dijkstra_t*)dijkstra;
	_Bool ret = false;
	d->graph->capacity = e;
	enlarge_graph(d->graph);
	 
	for(int i = 0; i < e; i++){
	d->graph->edges[i].from = edges[i][0];
	d->graph->edges[i].to = edges[i][1];
	d->graph->edges[i].cost = edges[i][2];
	}
	d->graph->num_edges = e;
	int m = -1;
	int q = 0;
	while (q < d->graph->num_edges){
	const edge_t *const e = &(d->graph->edges[q]);
	m = m < e->from ? e->from : m;
	m = m < e->to ? e->to : m;
	++q;
	}
	/*for (int i = 0; i < d->graph->num_edges; ++i) {
	const edge_t *const e = &(d->graph->edges[i]);
	m = m < e->from ? e->from : m;
	m = m < e->to ? e->to : m;
	}*/	
	m += 1;
	d->nodes = (node_t*)malloc(sizeof(node_t) * m);
	if (d->nodes) {
		d->num_nodes = m;
		int f = 0;
		while(f < m){
			d->nodes[f].edge_start = -1;
			d->nodes[f].edge_count = 0;
			d->nodes[f].parent = -1;
			d->nodes[f].cost = -1;
			++f;
		}
		/*for (int i = 0; i < m; ++i) {
			d->nodes[i].edge_start = -1;
			d->nodes[i].edge_count = 0;
			d->nodes[i].parent = -1;
			d->nodes[i].cost = -1;
		}*/
		int x = 0;
		while(x < d->graph->num_edges){
			int cur = d->graph->edges[x].from;
			if (d->nodes[cur].edge_start == -1) {
				d->nodes[cur].edge_start = x;
			}
			d->nodes[cur].edge_count += 1;
			++x;
		}
		/*for (int i = 0; i < d->graph->num_edges; ++i) {
			int cur = d->graph->edges[i].from;
			if (d->nodes[cur].edge_start == -1) { // first edge
				d->nodes[cur].edge_start = i; // mark the first edge in the array of edges
			}
			d->nodes[cur].edge_count += 1; // increase number of edges
		}*/
		ret = true;
	}
